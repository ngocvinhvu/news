...
class CategoryResource(BaseResource):
    ...
    async def post(self, payload: CategoryIn | List[CategoryIn],
                   request: GetRequest = Depends(GetRequest),
                   current_user=None):
        return await super(CategoryResource, self).post(payload, current_user=current_user)

    async def patch(self, payload: CategoryIn | List[CategoryIn] = None,
                    current_user=Depends(authenticate_admin),
                    request: GetRequest = Depends(GetRequest),
                    model_id: str | None = None):
        return await super(CategoryResource, self).patch(payload=payload,
                                                         request=request,
                                                         model_id=model_id,
                                                         current_user=current_user)
    ...
    async def delete(self, payload: DeleteRequest | List[DeleteRequest] = None,
                     ...
                     request: GetRequest = Depends(GetRequest),
                     model_id: str | None = None):
        return await super(CategoryResource, self).delete(payload=payload,
                                                          request=request,
                                                          model_id=model_id,
                                                          current_user=current_user)
